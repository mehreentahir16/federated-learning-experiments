...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)): 
        ...
        for idx in idxs_users:
            if count < update_number_clients:
                loss, lsum, model[idx], alpha[idx] = local_model[idx].update_weights(model[idx], global_round=epoch, alpha=alpha[idx], theta=copy.deepcopy(theta))
                local_losses.append(copy.deepcopy(loss))
                local_sum.append(copy.deepcopy(lsum))
            else:
                local_theta = copy.deepcopy(model[idx].state_dict())
                for key in local_theta.keys():
                    local_theta[key] = local_theta[key] + (1/args.rho) * alpha[idx][key]
                loss, lsum, model[idx], alpha[idx] = local_model[idx].update_weights(model[idx], global_round=epoch, alpha=alpha[idx], theta=copy.deepcopy(local_theta))
            count += 1
