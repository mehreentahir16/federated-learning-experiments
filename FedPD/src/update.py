    ...
    def update_weights(self, model, global_round, alpha, theta):
        ...
        for iter in range(E):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                loss.backward()
                model_weights_pre = model.state_dict()  
                for name, param in model.named_parameters():
                    if param.requires_grad == True:
                        param.grad = param.grad + (alpha[name] + self.args.rho * (model_weights_pre[name]-theta[name]))
                optimizer.step()
        ...
        for key in alpha.keys():
            alpha[key] = alpha[key] + self.args.rho * (weights[key]-theta[key])
        for key in alpha.keys():
            local_sum[key] = weights[key] + (1/self.args.rho) * alpha[key]
        return sum(epoch_loss) / len(epoch_loss), local_sum, model, alpha
